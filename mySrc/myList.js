function configList(navConfig){
    var res = {};
    res = setConfig(navConfig);
    return res;
}
function myList_Com(config){
...
myList_Com.idSeed = 0,
myList_Com.prototype={
    constructor:myList_Com,
    ...
    generateHtml:function(){
        ...
        if(!self.dom){
            var myConStr = "";
            myConStr = "<div class='"+this.class+" ' id='"+this.id+" '>"+this.html+"<div class='myList_Tep'></div><div  class='loadingIcon'></div><a src ='#' class = 'loadingError'></a></div>";
            self.dom = $(myConStr);
        }
    },
    ...
    generateItemHtml:function(){
        ...
        if(!self._innerTplEle ){
            self._innerTplEle = self.dom.find(".myList_Tep");
        }
    ...
    refreshDataStore:function(){
        ...
        if(!self.itemStore[self.listPage]){
            ...
            if(self.listPage ==1){
                self._innerTplEle.html(currentItem$)
            }else{
                setTimeout(function(){
    ...
    "dataUpInfo" :function(url,type){
        ...
        if(!self.loaded){
            ...
            switch(type){
                ...
                case "group":
                case "product":
                    itemTpl =  listStore[type](self);
                    break;
                case "companyDis":
                case "infosDis":
                case "showDis":
                case "productDis":
                    self.disType = type.slice(0,type.length-3);
                    break
                case "imageCollect":
                case "blogCollect":
                case "productCollect":
                ...
                case "showCollect":
                case "newsCollect":
                    self.collectType = type.slice(0,type.length-7);
                    break
            }
            if(itemTpl){
