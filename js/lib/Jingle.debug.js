...
Jingle.Menu = (function(J,$){
    ...
    var hideMenu = function(duration,callback){
        ...
        if(transition == 'overlay'){
            $aside.css({"-webkit-transform" : "translateX("+translateX+")"})
            _finishTransition();
...
;(function(J,$){
    ...
    function Refresh(selector,type,callback){
        var iscroll, scroller,refreshEl,pullIconEl,pullLabelEl, downIconEl,downLabelEl,topOffset,isPullDown,isUp
        options = {
            selector : undefined,
        ...
        var _init = function(opts){
            ...
            /*          ,pullIconEl,pullLabelEl, downIconEl,downLabelEl,*/
            switch ( options.type){
                case "pullDown":
                    refreshEl = $(refreshTpl).prependTo(scroller);
                    pullIconEl = refreshEl.find('.refresh-icon');
                    pullLabelEl=refreshEl.find('.refresh-label');
                    isPullDown = true;
                    isUp = false;
                    break;
                case "both":
                    ...
                    downIconEl = refreshEl.find('.refresh-icon');
                    downLabelEl=refreshEl.find('.refresh-label');
                    isPullDown = true;
                    isUp = true;
                    break;
                default :
        ...
        var _excuteScroll = function(opts){
            return J.Scroll(opts.selector,{
                ...
                onScrollMove : function(){
                    if (this.y > opts.minPullHeight && isPullDown && isPullDown&& !pullIconEl.hasClass(opts.onReleaseIcon)) {
                        pullIconEl.addClass(opts.onReleaseIcon);
                        pullLabelEl.html(opts.releaseText);
                        this.minScrollY = 0;
                    } else if (this.y < opts.minPullHeight && isPullDown && isPullDown&& pullIconEl.hasClass(opts.onReleaseIcon)) {
                        downIconEl.removeClass(opts.onReleaseIcon);
                        downLabelEl.html(opts.pullText);
                        this.minScrollY = -topOffset;
                    }else if (this.y < (this.maxScrollY - opts.minPullHeight) && isUp && !downIconEl.hasClass(opts.onReleaseIcon)) {
                        downIconEl.addClass(opts.onReleaseIcon);
                        downLabelEl.html(opts.releaseText);
                ...
                onScrollEnd : function(){
                    if(isPullDown&&pullIconEl.hasClass(opts.onReleaseIcon)){
                        pullIconEl.removeClass(opts.onReleaseIcon).removeClass(opts.onPullIcon).addClass(opts.onRefreshIcon);
                        pullLabelEl.html(opts.refreshText);
                        var _this = this;
                        setTimeout(function(){//解决在chrome下onRefresh的时候文本无法更改的问题。奇怪的问题！
                            opts.pullCallback.call(_this);
                        },1); 
                    }

                    if(isUp&&downIconEl.hasClass(opts.onReleaseIcon)){
                        downIconEl.removeClass(opts.onReleaseIcon).removeClass(opts.onPullIcon).addClass(opts.onRefreshIcon);
                        //downLabelEl.html(opts.refreshText);
                        downLabelEl.html("正在载入");
                        var _this = this;
                        setTimeout(function(){//解决在chrome下onRefresh的时候文本无法更改的问题。奇怪的问题！
                            opts.upCallback.call(_this);
                        },1);
                    }
                ...
                onRefresh: function () {
                    if(isPullDown){
                        pullIconEl.removeClass(opts.onRefreshIcon).addClass(opts.onPullIcon);
                        pullLabelEl.html(opts.pullText);
                    }
                    if(isUp){
                        downIconEl.removeClass(opts.onRefreshIcon).addClass(opts.onPullIcon);
                        downLabelEl.html(opts.pullText);
                    }
